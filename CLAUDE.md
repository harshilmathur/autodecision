...
/autodecision --team --skip-clarify "..."  # skip the Phase 2.5 question round
```

Runs the council as a persistent Claude Code Agent Team instead of fire-and-forget subagents. The 5 personas become teammates you can message directly (Shift+Down), and a new Phase 2.5 CLARIFY lets each persona ask 1–3 clarifying questions about your business context before simulating. Requires `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` in your `settings.json` env block and Claude Code v2.1.32+. If the env var is missing, the orchestrator falls back to standard subagent mode. See `skills/autodecision/references/team-mode.md` for prerequisites, architecture, and cleanup.
...
| `--team` | Opt in to Agent Teams council: 5 persistent teammates you can message directly. Adds Phase 2.5 CLARIFY where teammates ask persona-specific clarifying questions. Experimental — requires `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` and Claude Code v2.1.32+. See `skills/autodecision/references/team-mode.md`. |
| `--skip-clarify` | (Team mode only) Skip Phase 2.5 CLARIFY. Ignored without `--team`. |
...
├── agents/                           # Persona definitions (team mode + reusable subagents)
│   ├── optimist.md, pessimist.md, competitor.md,
│   └── regulator.md, customer.md
...
└── skills/autodecision/
    ...
    └── references/
        ...
        ├── persona-council.md        # 5 personas + Judge + subagent protocol + team-mode spawning
        ├── team-mode.md              # Agent Teams opt-in architecture and prerequisites
        ...
        │                             # + clarify-team, simulate-team, critique-team (team-mode variants)
        └── templates/                # pricing, expansion, build-vs-buy, hiring

.claude/                              # DERIVED — generated by scripts/sync.sh
├── agents/                           # Mirrors claude-plugin/agents/
...
Team mode is an opt-in execution path that runs the council as a Claude Code Agent Team. Personas become persistent teammates with direct user channels and inter-teammate messaging. The JSON artifact contract downstream of Phase 3 is identical to non-team mode, so the Convergence Judge, the brief schema, and the Phase 8.5 validator are unchanged.

- **Prerequisites:** Claude Code v2.1.32+, `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` in `settings.json`, and the `--team` flag on the command.
- **Activation:** Phase 0 SCOPE parses `--team` and writes `"team_mode": true` into `config.json`. A prerequisite check verifies the env var; if missing, the run falls back to standard subagent mode with a warning.
- **New phase:** Phase 2.5 CLARIFY (skippable with `--skip-clarify`) — teammates ask persona-specific clarifying questions about your business context; the lead batches them into one `AskUserQuestion`.
- **Phase variants:** `phases/simulate-team.md` and `phases/critique-team.md` replace their standard counterparts in team mode. All other phases are reused verbatim.
- **Full spec:** `skills/autodecision/references/team-mode.md`.
...
| Personas (prompts + council structure) | `claude-plugin/skills/autodecision/references/persona-council.md` |
| Persona content (reusable as subagent or teammate) | `claude-plugin/agents/{short-tag}.md` |
...
| Team mode routing / lifecycle | `claude-plugin/skills/autodecision/references/team-mode.md` (architecture) and `engine-protocol.md` "Team Mode Routing" section (loop routing) |
| Team-mode Phase 2.5 / 3 / 4 behavior | `claude-plugin/skills/autodecision/references/phases/clarify-team.md`, `simulate-team.md`, `critique-team.md` |
