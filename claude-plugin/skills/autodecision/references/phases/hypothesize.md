...
where applicable and only invent new IDs for genuinely novel effects (specialist
insights, tiered effect variants, novel mechanisms the seeded list doesn't cover).

The 4-6 range is intentional: enough seeded IDs to cover the obvious mechanism
space and give synthesis exact-ID matches to merge on, but not so many that
personas treat them as a target floor (8+ seeds tends to push personas toward
producing exactly 8 effects, which inflates singleton islands).
